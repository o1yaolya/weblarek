...
модальные окна (детализация, корзина, оформление заказа);
обработку пользовательских действий;
передачу событий в презентер.

HeaderView - отображение иконки корзины со счётчиком.

GalleryView - отображение списка товаров в каталоге.

BasketView - отображение списка товаров в корзине, общей стоимости, кнопки оформления заказа.
...
Родительский класс: 
Card - общий функционал для всех карточек товаров (рендеринг базовых элементов: изображение, название, цена).

Дочерние классы:

SimpleCard — карточка с минимальным набором элементов (изображение, название, цена), используется на главной странице в сетке товаров.

DetailedCard — расширенная карточка (изображение, название, цена, краткое описание),используется в разделе каталога, где товары отображаются списком. 

BasketCard — карточка в корзине (название, цена)


- Классы для форм

OrderConfirmation - форма для отображения окна успешного оформления заказа(сумма списания).

Родительский класс: 
BaseForm - Содержит базовый функционал, необходимый для работы с любыми формами на сайте.

Дочерние классы:

OrderForm - для первого шага оформления заказа (выбор способа оплаты и ввод адреса доставки) на основе BaseForm

ContactsForm - форма для ввода контактных данных (email, телефон).
